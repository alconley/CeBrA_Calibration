...
Eu152_energy = np.array([121.7817,244.6975,344.2785,411.1163,443.9606,778.904,867.38,964.057,1085.837,1112.076,1212.948,1299.142,1408.006]) #in keV
Eu152_intensity = np.array([28.53,7.55,26.5,2.234,2.827,12.93,4.23,14.51,10.11,13.67,1.415,1.633,21.005]) #in %

Eu152_dic = dict(zip(Eu152_energy, Eu152_intensity))
...
while i <= 4:  
       ...
       with open("./data_files/detector_" + str(i) + "_calibrated_data.txt") as f:
              ...
              for line in stripped:
                     ...
                     for key in Eu152_dic:
                            if(key >= bot_err and key <= top_err):
                                   dict[key] = [float(energy), float(energy_error), float(width),
                                                float(width_error), float(volume), float(volume_error), Eu152_dic[key]]
